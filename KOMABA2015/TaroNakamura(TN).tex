...
\item $\lVert xy\rVert \leq \lVert x\rVert \lVert y\rVert$ %AとCの元入り混じっているのでx,y,\alphaはどこの元か書いたほうがいいのでは...?
\item $\left(x^*\right)^*=x$
\item $\left(x+y\right)=x^*+y^*$ %おかしい*ついてない
\item $\left(xy\right)^*=y^*a^*$ %おかしい
...
\sup_{\lambda \in \Lambda}<\infty  \}\] %おかしい
...
\[A=\sum_{\lambda \in \Lambda} \oplus A_{\lambda}\] %おかしい
と表す．
...
\[\left(de\right)e+ede=de\] %おかしいdではなくD?
この式の両辺から$2eDe$を引けば
